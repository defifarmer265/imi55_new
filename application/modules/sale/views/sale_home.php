                    ...
                    <tbody id="history">
                      ...
                      foreach ($sale as $_s => $ds) { ?>
                        <tr align="center">
                          ...
                          <td><?=$ds['num_userD']; ?></td>
                          <td><?=$ds['num_userM']; ?></td>
                          <td> <button type="button" class="btn btn-secondary btn-sm" onClick="detail('<?=$ds['id']?>')">
...
<script>
	...
	function detail(id) 
	{
		window.location.href = '<?=base_url()?>sale/salelist/calsale/'+id;
	}
	...
	function edit_pass() 
	{
		...
		swal({
			...
			.then((willDelete) => {
				if (willDelete) {
					$.ajax({
							url: '<?=base_url()?>sale/salelist/edit_pass',
							type: 'POST',
							dataType: 'json',
							data: {
								id: id,pass:pass
	...
	function sel_status() 
	{
		...
		var status = $( "#sale_status option:selected" ).val();
		 $.ajax({
			url: '<?=base_url()?>sale/salelist/get_sale',
			type: 'POST',
			dataType: 'json',
			data: {
				status:status
			...
			if (res.code == 1) {
				...
				if(res.data.length >= 1 ){
					...
					for(var i=0; i < conut; i++){
						...
						content += '<td> <button type="button" class="btn btn-secondary btn-sm" onClick="detail(`'+sale[i]['id']+'`)"><i class="fa fa-file"> รายละเอียด</i></button></td>';
  ...
  function edit_status(id,status) 
    ...
    swal({
        ...
        .then((willDelete) => {
            if (willDelete) {
                $.ajax({
                        url: '<?=base_url()?>sale/salelist/edit_status',
                        type: 'POST',
                        dataType: 'json',
                        data: {
                            id: id,status:status
  ...
  function cre_sale() 
    ...
    $.ajax({
        url: '<?=base_url()?>sale/salelist/cre_sale',
        type: 'POST',
        dataType: 'json',
        data: data,
      })
