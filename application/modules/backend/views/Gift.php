...
class Gift extends MY_Controller
{
        ...
        if ($limit_user != '' && $code != '') {
            ...
            if ($tb_gift->num_rows() == 0) {
                ...
                if ($this->db->insert('tb_gift', $data)) {
                    ...
                    $hostname = $this->backend_library->select_sql('157.245.207.178', 'imi_center', $sql);
                    $hostname = $hostname[0]['hostname'];

                    $sql = "SELECT database_name FROM tb_listhost WHERE webname = '" . $webname . "' AND status = 1";
                    $dbname = $this->backend_library->select_sql('157.245.207.178', 'imi_center', $sql);
    ...
    public function receive($id){
      ...
      $data['receive_gift']= $this->db->select('log_gift_voucher.id,, log_gift_voucher.user_id,log_gift_voucher.receive,log_gift_voucher.time_receive ,log_gift_voucher.user_id')
                    ->where('log_gift_voucher.gift_id',$id)
                    ->where_in('log_gift_voucher.receive',array(1, 2))
