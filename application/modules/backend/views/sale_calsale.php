...
<div class="right_col" role="main">
  ...
  <div class="row">
    <div class="col-md-11 col-sm-11 ">
      <div class="x_panel">
        <div class="x_title">
          <h2><a href="<?=base_url()?>backend/sale/home">SALE</a> / <?=$sale->username?> (<?=$sale->name?>)</h2>
	...
	function report_sale(){
		...
		swal({
			...
			.then((willDelete) => {
				...
				if (willDelete) {
					$.ajax({
							url: '<?=base_url()?>backend/sale/report_sale',
							type: 'POST',
							dataType: 'json',
							data: {
								d1:d1,d2:d2,id:id
						...
						.done(function(res) {
							...
							if (res.code == 1) {
								...
								for(var i=0; i < conut; i++){
									...
									content += '<td>'+calsale[i]['create_time']+'</td>';
									if(calsale[i]['deposit'] != 0){		content += '<td>'+calsale[i]['deposit']+'</td>';}else{content += '<td class="text-left"> null </td>';}
...
//		var nf = $('#num_free').val();
		...
		swal({
			...
			.then((willDelete) => {
				...
				if (willDelete) {
					$.ajax({
							url: '<?=base_url()?>backend/sale/calculate_',
							type: 'POST',
							dataType: 'json',
							data: {
