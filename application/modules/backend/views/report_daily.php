...
<div class="right_col" role="main">
  ...
  <div class="row">
    <div class="col-md-12 col-sm-12 ">
      <div class="x_panel">
        <div class="x_title">
          <h2>รายการฝาก - ถอน รายวัน <small></small></h2>
          <div class="clearfix"></div>
        </div>
        <div class="row ">
          <div class="col-sm-2 ">
...
<script>
  function select_daily() {
    ...
    var tm2 = $('#time2').val();
    $.ajax({
        url: '<?= base_url() ?>backend/daily/rp_us',
        type: 'POST',
        dataType: 'json',
        data: {
          dt1: dt1,
      ...
      .done(function(res) {
        if (res.code == 1) {
          if (res.data.length > 0) {
            ...
            var sum = '<tr><td colspan="1" class="text-right">รวม</td><td class="text-right">'+nb(tt_user)+'</td><td class="text-right">'+nb(tt_n_dp)+'</td><td class="text-right">'+nb(tt_dp.toFixed(2))+'</td><td class="text-right">'+nb(tt_n_wd)+'</td><td class="text-right">'+nb(tt_wd.toFixed(2))+'</td><td class="text-right">'+nb(tt_all.toFixed(2))+'</td></tr>';
