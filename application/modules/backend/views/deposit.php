                  ...
                  <table id="datatable-responsive" class="table table-striped table-bordered nowrap" cellspacing="0" width="100%" >
                    <thead style="background-color: #2a3f54;color: #fff;">
                      <tr align="center">
                        ...
                        <th width="2%">Id</th>
                        <th width="5%">Close</th>
                        <th width="10%">วันที่ฝาก </th>
                        <th width="5%">เวลาฝาก </th>
                        <th width="10%">ฝาก</th>
                        <th>Status</th>
                        <th>เวลาทำรายการ</th>
                       
                        <th width="5%">ฝากจาก</th>
                        <th>รายละเอียด</th>
                    ...
                    <tbody>
                      ...
                      if ( empty( $statement ) ) {
                        ?>
                      <tr>
                        <td colspan="21" style="background-color:#FFFF99" ><center>
                            No data
                          </center></td>
                      </tr>
                      <?php
                      ...
                      <tr align="center">
                        <td><?php echo $i;?></td>
                        <td><?=$state['id']?></td>
                        <td><button class="" onClick="closeDPS('<?=$state['id']?>')"><i class="fa fa-remove"></i></button></td>
                        <td><?=date('d-M-y',$state['datetime'])?></td>
                        <td><?=date('H:i',$state['datetime'])?></td>
                        <td style="text-align: right;"><?=number_format($state['deposit'],2)?></td>
                      
                        <td>
            <?php if ( $state[ 'status' ] == 2 ) {?>
              <span style="cursor: pointer;" onClick="gethistory('<?=$state['user']?>')">
                รหัส : <?=substr($state['user'],-6)?> <br>  Bank : <?=substr($state['usbk'],-6).' '.$state['bank_short']?> <br>
                เบอร์ : <?=$state['username']?>
              </span>
            <?php } else { ?>
              <button onClick="mod_addcredit(<?=$state['id']?>,<?=$state['deposit']?>)">+ เพิ่ม </button>
             <?php } ?>
            </td>
                        
                        <td><?=date('d-m-Y H:i',$state['dateCreate'])?></td>
                        <td><?=$state['from_name']?></td>
                        <td><?=$state['note']?></td>
                      </tr>
                      <?php $i++; } } ?>
                    </tbody>
