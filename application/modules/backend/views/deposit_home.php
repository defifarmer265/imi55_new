                        ...
                        <tr id="<?= $state['id'] ?>">
                          ...
                          <td width="10%" align="center">
                            <div class="col-md-12" style="font-size: 15px;">
                              <button class="badge badge-success " onClick="add_credit_stm('<?= $state['id'] ?>','DPS<?= substr(("000000" . (intval($state['id']))), -6) ?>','<?= number_format($state['deposit'], 2) ?>')"> <i class="fa fa-check">&nbsp;&nbsp;เพิ่ม&nbsp;&nbsp;</i> </button>
                            </div>
                            <div class="col-md-12">
                              <button class="badge badge-warning" onClick="edit_status_dps('<?= $state['id'] ?>','5','DPS<?= substr(("000000" . (intval($state['id']))), -6) ?>')"> <i class="fa fa-remove">&nbsp;&nbsp;ซ่อน &nbsp;</i> </button>
                            </div>
                            <div class="col-md-12">
                              <button class="badge badge-danger" onClick="edit_status_dps('<?= $state['id'] ?>','4','DPS<?= substr(("000000" . (intval($state['id']))), -6) ?>')"> <i class="fa fa-remove"> ยกเลิก</i> </button>
                            </div>
                          </td>
                        </tr>
                    <?php $i++;
      ...
      <div class="modal-body">
        ...
        <div class="col-md-12 col-sm-12  form-group has-feedback text-dark">
          <b>
            ...
            <h6>ยอดเงิน : <span id="m_stmamount"></span> บาท</h6 </b> </div> <div class="col-md-12 col-sm-12  form-group has-feedback">
...
<script>
  function type_stm() {
    var type = $('#type').val();
    $.ajax({
      url: '<?= base_url() ?>backend/deposit/get_type_stm',
      type: 'POST',
      dataType: 'json',
      data: {
        status: type
      ...
      if (stm.length >= 1) {
        for (var i = 0; i < conut; i++) {
          ...
          if (type == 5) {
            ...
            history_stm += '<button class="badge badge-success "  onClick="add_credit_stm(`' + stm[i].id + '`,`DPS' + stm[i].id + '`,`' + stm[i].deposit + '`)"> <i class="fa fa-check">&nbsp;&nbsp;เพิ่ม&nbsp;&nbsp;</i> </button>';
            history_stm += '</div> ';

            history_stm += '<div class="col-md-12">';
            history_stm += '<button class="badge badge-danger" onClick="edit_status_dps(`' + stm[i].id + '`,`4`,`DPS' + stm[i].id + '`,`' + stm[i].deposit + '`)" > <i class="fa fa-remove"> ยกเลิก</i> </button>';
            history_stm += '</div>';
          } else if (type == 4) {
            history_stm += '<div class="col-md-12">';
            ...
            history_stm += '</div> ';
            history_stm += '<div class="col-md-12">';
            history_stm += '<button class="badge badge-warning" onClick="edit_status_dps(`' + stm[i].id + '`,`5`,`DPS' + stm[i].id + '`,`' + stm[i].deposit + '`)" > <i class="fa fa-remove">&nbsp;&nbsp;ซ่อน &nbsp;</i> </button>';
            history_stm += '</div>';
            history_stm += '<div class="col-md-12">';
            history_stm += '<button class="badge badge-danger" onClick="edit_status_dps(`' + stm[i].id + '`,`4`,`DPS' + stm[i].id + '`,`' + stm[i].deposit + '`)" > <i class="fa fa-remove"> ยกเลิก</i> </button>';
            history_stm += '</div>';
  ...
  function add_credit_stm2() {
    ...
    $.ajax({
      url: '<?= base_url() ?>backend/deposit/add_credit_check_user',
      type: 'POST',
      dataType: 'json',
      data: {
        user: user_,
      ...
      if (res.code == 1) {
        var us = res.data;
        $("input[name=add_user_id]").val(us.user);
        ...
        detail_stmcf += '<td colspan="3">&nbsp;&nbsp;';
        detail_stmcf += '<span> ' + us.user + ' </span></td>'
        detail_stmcf += '</tr>';

        detail_stmcf += '<tr><td  align="right"> เครดิต : </td><td>&nbsp;&nbsp;<span style="font-size: 14px;" class="badge badge-info">' + us.credit + '&nbsp; ฿</span></td><td  align="right"> เบอร์ : </td><td>&nbsp;&nbsp;' + us.username + '</td></tr>';
        detail_stmcf += '<tr><td  align="right"> รหัส : </td><td>&nbsp;&nbsp;<span>' + stmid_show + '</span></td><td  align="right">ฝากเข้า : </td>';
        detail_stmcf += '<td><img  src="<?= base_url() ?>public/tem_frontend/img/mapraw_icon/bank/' + us.webApi + '.png" style="border: 1px solid #FFF;border-radius:5px" class="img-fluid" width="30px"><span> ' + us.webName + ' [' + us.webBank + '] </span></td></tr>';


        detail_stmcf += '<tr><td  align="right"> รายละเอียด :</td> <td colspan="3">&nbsp;&nbsp;' + us.note + '</td></tr>';
        detail_stmcf += '<tr><td  align="right"> ยอดฝาก :</td> <td colspan="3" >&nbsp;&nbsp;<span style="font-size: 18px;" class="badge badge-secondary">' + us.amount + '&nbsp; ฿ </span> <i class="fa fa-check text-success"></i></td></tr>';
        detail_stmcf += '<tr><td align="right"> โปร :</td> <td colspan="3" style="font-size: 1.3em;color: red;background: blanchedalmond;"><b>' + (us.promo).name + '</b></td></tr>';
        detail_stmcf += '<tr><td align="right"> outstanding :</td> <td colspan="3" style="font-size: 1.3em;color: red;background: blanchedalmond;"><b>' + (us.outstanding) + '</b></td></tr>';
        detail_stmcf += '<tr><td align="center" colspan="4" style="font-size: medium;"';
                        ((us.promo).id==0)?detail_stmcf += ' hidden':detail_stmcf += ''; 
        detail_stmcf += '>';
        
        detail_stmcf += 'ไม่เคลียโปร :: <input style="width: 1.2em;height: 1.2em;" type="radio" name="chcutTurn" value="0" checked />&nbsp;&nbsp;|&nbsp;&nbsp;';
        detail_stmcf += 'เคลียโปร :: <input style="width: 1.2em;height: 1.2em;" type="radio" value="1" name="chcutTurn"/>';
        detail_stmcf += '</td></tr>';
        detail_stmcf += '</table>';
  ...
  function addcredit() {
    ...
    var chcutTurn = $("input[name=chcutTurn]:checked").val();
    console.log(chcutTurn);
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          $('#cover-spin').show();
          $.ajax({
            url: "<?= base_url() ?>backend/deposit/addcredit",
            type: 'post',
            data: {
              state_id: state_id,
              user: user,
  ...
  function detail_stmcf(stm_id) {
    ...
    $.ajax({
        url: '<?= base_url() ?>backend/deposit/get_stm',
        type: 'POST',
        dataType: 'json',
        data: {
          stm_id: stm_id
  ...
  function edit_status_dps(dps_id, status, dpsid_show) {
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          $.ajax({
            url: "<?= base_url() ?>backend/deposit/edit_status_dps",
            type: 'post',
            data: {
              dps_id: dps_id,
              status: status
