										...
										<tbody>
											...
											?>
													<tr align="center">
														...
														<td><?= $wd['account'] ?></td>
														<td><?= $wd['bank_short'] ?></td>
														<td class="text-right"><b><?php $am1 = substr($wd['amount'], 0, -3);
																					echo number_format($am1) ?></b></td>
														...
														<td><?php
															...
															?>
																...
																<button class="btn btn-sm btn-danger" onClick="wd_accept('reject','<?= $wd['id'] ?>')" title="ยกเลิกรายการ"> <i class="fa fa-remove"></i> </button>
															<?php
															} else if ($wd['status'] == 2) {
															?>
															...
															?>
																<button class="btn btn-sm btn-success" onClick="wd_accept('confrim','<?= $wd['id'] ?>')" title="ยืนยันรายการฝาก"> <i class="fa fa-check"></i> </button>
															<?php
															} else if ($wd['status'] == 6) {
				...
				<div class="col-md-12 col-sm-12  form-group has-feedback">
					<div class="row">
						<div class="col-md-6">
							...
							<span id="api_history_WD"></span>
						</div>
						<div class="col-md-6">
							<h6>จากรายการฝาก</h6>
							<span id="state_dw"></span>
						</div>
						...
						<div class="col-md-9 col-sm-9 ">
							...
							<select name="bw_id" id="bw_id" class="form-control">
								...
								foreach ($bankweb as $_w => $bw) {
									...
									<option <?= $styauto ?> value="<?= $bw['id'] ?>">
										<?php echo $bw['name'] . ' [' . $bw['bank_short'] . '] ' . number_format($bw['sum']); ?>
									</option>
...
<script>
	function accept(id) {
		...
		$.ajax({
			...
			for (var c = 0; c < api_count; c++) {
				if (api_story[c]['success'] == 1) {
					var cls = 'table-success';
				} else if (api_story[c]['success'] == 2) {
					var cls = '';
				} else {
					var cls = 'table-danger';
				}
				...
				if (api_story[c]['Amount'] > 0) {
					api_history_WD += '<td class="text-right">' + api_story[c]['Amount'] + '</td>';
				} else {
					api_history_WD += '<td class="text-right"> - </td>';
				}
				if (api_story[c]['Amount'] < 0) {
					api_history_WD += '<td class="text-right">' + api_story[c]['Amount'] + '</td>';
				} else {
					...
					swal({
						...
						if (willDelete) {
							$.ajax({
								...
								//							$('#cover-spin').hide();
								console.log(res);
								if (res.data.status.code == 1000 && res.code == '014') {
									const wrapper = document.createElement('div');
									wrapper.innerHTML = "<img src='<?php echo $this->config->item('tem_frontend') ?>/img/mapraw_icon/bank/" + res.code + ".png' width='80px'><br> <b>โอนเงินไปยัง : " + res.data.data.accountToName + "</b><br><b>เลขบัญชี : " + res.data.data.accountTo + " ธนาคาร <img src='<?php echo $this->config->item('tem_frontend') ?>/img/mapraw_icon/bank/" + res.data.data.accountToBankCode + ".png' width='50px'></b><br><b style='color:blue'>จำนวนเงิน " + res.amt + " บาท</b>";
									swal({
											title: "ถอน",
											content: wrapper,
											buttons: true,
								...
								} else if (res.code == '002' && res.data.status == true) {
									swal({
										icon: "error",
										text: res.data.message.name,
									});
									const wrapper = document.createElement('div');
									const acc_bank = res.data.message.accnum;
									wrapper.innerHTML = "<img src='<?php echo $this->config->item('tem_frontend') ?>/img/mapraw_icon/bank/" + res.code + ".png' width='80px'> <br><b>โอนเงินไปยัง : " + res.data.message.name + "</b><br><b>เลขบัญชี : " + acc_bank.replace(/-/g, "") + " ธนาคาร <img src='<?php echo $this->config->item('tem_frontend') ?>/img/mapraw_icon/bank/" + res.data.accountToBankCode + ".png' width='50px'></b><br><b style='color:blue'>จำนวนเงิน " + res.amt + " บาท</b>";
									swal({
											title: "ถอน",
											content: wrapper,
											buttons: true,
	...
	function confirm_withdraw(withdraw_id, status, num) {
		var bank_web_id = document.getElementById("bank_web" + num).value;
		swal({
				title: "ยืนยันการถอน",
				icon: "warning",
				buttons: true,
