...
<script>
  function select_daily() {
    ...
    $.ajax({
        url: '<?= base_url() ?>backend/Report_award/rp_us',
        type: 'POST',
        dataType: 'json',
        data: {
          dt1: dt1,
      ...
      .done(function(res) {
        if (res.code == 1) {
          if (res.data.length > 0) {
            ...
            var wt  = 'withdraw';
            var dp  = 'deposit';
            let url = '<?= base_url() ?>backend/report_award/report_rs/';
            for (var i = 0; i < conut; i++) {
              ...
              content += '<td>';
              content += '  <a href="' +url   +wd[i]['date'] + '/'+ dp + '  " class="" target="_blank"><i class="fa fa-money"></i> รายละเอียดฝาก</a>';
              content += ' | <a href="' +url   +wd[i]['date'] + '/'+ wt + '  " class="" target="_blank"><i class="fa fa-money"></i> รายละเอียดถอน</a>';
              content += '</td>';
            ...
            var sum = '<tr><td colspan="1" class="text-right">รวม</td><td class="text-right">'+nb(tt_n_dp)+'</td><td class="text-right">'+nb(tt_dp.toFixed(2))+'</td><td class="text-right">'+nb(tt_n_wd)+'</td><td colspan="1" class="text-right">'+nb(tt_wd.toFixed(2))+'</td><td></td></tr>';
