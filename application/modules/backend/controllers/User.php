...
class User extends MY_Controller
{
    ...
    public function sel_detail()
    {
        ...
        if ($user_r) {
            $user_r->credit = $this->get_credit($user_r->user);
            $user_r->dw = $this->db->select('id,from_unixtime(datetime, "%Y-%m-%dT%H:%i:%s" ) as datetime2,dateCreate,deposit,withdraw')
                ->where('user_id', $user_r->id)
                ->order_by('id', 'DESC')
                ->limit(10)
                ->get('tb_statement')
            ...
            $turn = $this->db->where('user_id', $user_r->id)->get('tb_turnover');
            if ($turn->num_rows() == 0) {
                $turn_n = 0;
                $date = 'dd-mm-YY';
            } else {
                $turn_n = $turn->row()->checkturn;
                $date = date("d-m-Y", $turn->row()->check_time);
            }
            // ========================   get user rank ======================
            $getrank =  $this->db->select('tb_user_rank.total_turnover,tb_rank.name,tb_rank.img_link')
                ->join('tb_rank', 'tb_rank.id = tb_user_rank.id_rank')
                ->where('tb_user_rank.user', $user_r->user)
                ->get('tb_user_rank');
            if ($getrank->num_rows() == 1) {
                $user_r->showrank = '
                            <div class="col-md-4 form-group text-right p-2 h6">
                                <span class="text-danger">ยอดเทิร์นรวม</span>
                            </div>
                            <div class="col-md-4 form-group text-center">
                                ' . $getrank->row()->total_turnover . '
                            </div>
                            <div class="col-md-4 form-group text-center">
                                  ' . $getrank->row()->name . '
                            </div>
                ...
                $this->db->insert('tb_user_rank', array("user" => $user_r->user, "total_turnover" => 0, "id_rank" => 1));
                $user_r->showrank = '
                            <div class="col-md-4 form-group text-right p-2 h6">
                                <span class="text-danger">ยอดเทิร์นรวม</span>
                            </div>
    ...
    public function edit_name()
    {
        if ($this->input->post('user') && $this->input->post('name')) {
            ...
            $user_r = $this->db->select('id,user,name')->where('user', $user)->get('tb_user')->row();
            $name_old = $user_r->name;
            if ($this->db->set('name', $name)->where('id', $user_r->id)->update('tb_user')) {
                ...
                $sql = "INSERT INTO log_edit_user_name (admin, user_id,detail,datetime) 
                        VALUES ('" . $this->session->userdata['admin']['username'] . "', '" . $user_r->user . "','" . 'old : ' . $name_old . ' new : ' . $name . "','" . $time . "')";
                if ($this->backend_library->query_sql($hostname, $dbname, $sql)) {

                
                } else {

                    $sql = "CREATE TABLE log_edit_user_name (
    ...
    public function edit_username()
    {
        ...
        if ($this->input->post('username') && $this->input->post('user')) {
            $user = $this->input->post('user');
            $username = $this->input->post('username');
            $username_old = $this->db->where('user',$user)->get('tb_user')->row()->username;
            if ($this->db->set('username', $username)->where('user', $user)->update('tb_user')) {
                ...
                $sql = "INSERT INTO log_edit_user_username (admin, user_id,detail,datetime) 
                VALUES ('" . $this->session->userdata['admin']['username'] . "', '" . $user  . "','" . 'old : ' . $username_old . ' new : ' . $username . "','" . $time . "')";
                if ($this->backend_library->query_sql($hostname, $dbname, $sql)) {

                   
                } else {

                    $sql = "CREATE TABLE log_edit_user_username (
                         id INT(10) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
                         admin VARCHAR(20) NOT NULL,
                         user_id VARCHAR(20) NOT NULL,
    ...
    public function edit_pass()
    {
        if ($this->input->post('user') && $this->input->post('password')) {
            ...
            $admin = $this->session->users['name'];
            $user_r = $this->db->select('*')->where('user', $user)->get('tb_user')->row();
            $password_old = $user_r->password;
            ...
            if ($cre_userAPI->Status == 1) {
                if ($this->db->set('password', $password)->where('id', $user_r->id)->update('tb_user')) {
                    ...
                    $sql = "INSERT INTO log_edit_user_pass (admin, user_id,detail,datetime) 
                            VALUES ('" . $this->session->userdata['admin']['username'] . "', '" . $user_r->user . "','" . 'old : ' . $password_old . ' new : ' . $password . "','" . $time . "')";
                    if ($this->backend_library->query_sql($hostname, $dbname, $sql)) {

                       
                    } else {

                        $sql = "CREATE TABLE log_edit_user_pass (
								id INT(10) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
								admin VARCHAR(20) NOT NULL,
								user_id VARCHAR(20) NOT NULL,
                                detail VARCHAR(100) NOT NULL,
        ...
        if ($this->input->post('user') && $this->input->post('status')) {
            ...
            if ($user_q->num_rows() == 1) {
                ...
                if ($this->db->set('status', $status)->where('id', $user_r->id)->update('tb_user')) {
                    ...
                    $sql = "INSERT INTO log_edit_user_status (admin, user_id,detail,datetime) 
                        VALUES ('" . $this->session->userdata['admin']['username'] . "', '" . $user_r->user . "','" . 'old : ' . $status_old . ' new : ' . $status_new . "','" . $time . "')";
                    if ($this->backend_library->query_sql($hostname, $dbname, $sql)) {
                       
                    } else {

                        $sql = "CREATE TABLE log_edit_user_status (
                            id INT(10) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
                            admin VARCHAR(20) NOT NULL,
                            user_id VARCHAR(20) NOT NULL,
                            detail VARCHAR(100) NOT NULL,
    ...
    public function edit_bank()
    {
        if ($this->input->post('bank_id') && $this->input->post('account') && $this->input->post('user')) {
            ...
            $account = $this->input->post('account');
            $bank_id = $this->input->post('bank_id');
            $user_r = $this->db->select('id,user')->where('user', $user)->get('tb_user')->row();
            ...
            if ($bu_r && $user_r) {
                ...
                if ($check->num_rows() == 0) {
                    ...
                    if ($this->db->where('id', $bu_r->id)->update('tb_user_bank', $arr_bank)) {
                        ...
                        $sql = "INSERT INTO log_edit_user_bank (admin, user_id,detail,datetime) 
                        VALUES ('" . $this->session->userdata['admin']['username'] . "', '" . $user_r->user . "','" . 'old : ' . $log_bu . ' new : ' . $account . $bankshort . "','" . $time . "')";

                        if ($this->backend_library->query_sql($hostname, $dbname, $sql)) {
                           
                        } else {

                            $sql = "CREATE TABLE log_edit_user_bank (
								id INT(10) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
								admin VARCHAR(20) NOT NULL,
								user_id VARCHAR(20) NOT NULL,
                                detail VARCHAR(100) NOT NULL,
    ...
    public function edit_group()
    {
        ...
        $user_r = $this->db->where('status', 1)->where('id', $user_id)->get('tb_user')->row();

        $Group_r = $this->db->get('tb_group')->result_array();
        $arr_group_old = [];
        $checkGroup = $this->db->select('tb_user_group.*,tb_group.name')
            ->join('tb_group', 'tb_user_group.group_id = tb_group.id  ')
            ->where('tb_user_group.user_id', $user_id)
            ->where('tb_user_group.status', 1)
            ->get('tb_user_group')->result_array();
        foreach ($checkGroup as $chgr) {
            array_push($arr_group_old, $chgr['name']);
        }
        $arr_group_new = [];

        //        print_r($this->input->post());die();
        foreach ($Group_r as $gr) {
            $chkGroup = $this->db->select('tb_user_group.*,tb_group.name')
                ->join('tb_group', 'tb_user_group.group_id = tb_group.id  ')
                ->where('tb_user_group.group_id', $gr['id'])
                ->where('tb_user_group.user_id', $user_id)
                ->get('tb_user_group');
            ...
            if (empty($this->input->post($gr['id']))) {
                ...
                if ($chkGroup->num_rows() == 0) {
                    array_push($arr_group_new, $gr['name']);
                    $arrbankGroup = array(
                        'group_id' => $gr['id'],
                        'user_id' => $user_id,
                        'status' => 1
                    ...
                    array_push($arr_group_new, $bGroup_r->name);
        ...
        $i = 0;
        $user_group_new = '';
        foreach ($arr_group_new as $key => $value) {
            if ($i + 1 == sizeof($arr_group_new)) {
                $user_group_new .= $value;
            } else {
                $user_group_new .= $value . ',';
            }
            $i++;
        ...
        $j = 0;
        $user_group_old = '';
        foreach ($arr_group_old as $key => $value) {
            if ($j + 1 == sizeof($arr_group_old)) {
                $user_group_old .= $value;
            } else {
                $user_group_old .= $value . ',';
            }
            $i++;
        ...
        $sql = "INSERT INTO log_edit_user_group (admin, user_id,detail,datetime) 
        VALUES ('" . $this->session->userdata['admin']['username'] . "', '" . $user_r->user . "','" . 'old : ' . $user_group_old . ' new : ' . $user_group_new . "','" . $time . "')";
        if ($this->backend_library->query_sql($hostname, $dbname, $sql)) {
            
        } else {

            $sql = "CREATE TABLE log_edit_user_group (
                 id INT(10) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
                 admin VARCHAR(20) NOT NULL,
                 user_id VARCHAR(20) NOT NULL,
    ...
    {
        if ($this->input->post('user') && $this->input->post('amount')) {
            $user = $this->input->post('user'); //USERS
            $amount_out = $this->input->post('amount'); //amout
            $amount = $this->get_credit($user);
            $agent = $this->session->users['username'];
    ...
    public function deposit($user, $amount, $credit_last)
    {
        ...
        if ($user_q->num_rows() == 1) {
            ...
            if ($cre_userAPI->Success == 1) {
                ...
                $dbname = $dbname[0]['database_name'];
                $time = time();
                $sql = "INSERT INTO log_edit_user_credit (admin, user_id,action,detail,datetime) 
                VALUES ('" . $this->session->userdata['admin']['username'] . "', '" . $user_r->user . "','up','" . 'old : ' . $credit_last . ' new : ' . $this->get_credit($user_r->user) . '  ' . 'amount : ' . $amount . "','" . $time . "')";

                if ($this->backend_library->query_sql($hostname, $dbname, $sql)) {
                   
                } else {

                    $sql = "CREATE TABLE log_edit_user_credit (
                        id INT(10) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
                        admin VARCHAR(20) NOT NULL,
                        user_id VARCHAR(20) NOT NULL,
    ...
    public function withdraw($user, $amount, $credit_last)
    {
        ...
        if ($user_q->num_rows() == 1) {
            ...
            if ($cre_userAPI->Success == 1) {
                ...
                $dbname = $dbname[0]['database_name'];
                $time = time();
                $detail = 'old : ' . $credit_last . ' new : ' . $this->get_credit($user_r->user) . ' new : ' . $credit_last;
                $sql = "INSERT INTO log_edit_user_credit (admin, user_id,action,detail,datetime) 
                VALUES ('" . $this->session->userdata['admin']['username'] . "', '" . $user_r->user . "','down','" . 'old : ' . $credit_last . ' new : ' . $this->get_credit($user_r->user) . '  ' . 'amount : ' . $amount . "','" . $time . "')";
                if ($this->backend_library->query_sql($hostname, $dbname, $sql)) {
            ...
            foreach ($query as $result_user_point1) {
                if ($amount_out <= $result_user_point1['point']) {
                    ...
                    if ($result_point) {
                        ...
                        $dbname = $dbname[0]['database_name'];
                        $time = time();
                        $sql = "INSERT INTO log_edit_user_point (admin, user_id,action,detail,datetime) 
                            VALUES ('" . $this->session->userdata['admin']['username'] . "', '" . $user . "', 'down','" . 'old : ' . $result_user_point1['point'] . ' new : ' .  $result_total . "','" . $time . "')";
                        ...
                        if ($this->backend_library->query_sql($hostname, $dbname, $sql)) {
                            ...
                            $sql = "CREATE TABLE log_edit_user_point (
								id INT(10) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
								admin VARCHAR(20) NOT NULL,
								user_id VARCHAR(20) NOT NULL,
    ...
    public function point_in()
    {
        ...
        if ($this->input->post('user') && $this->input->post('amount')) {
            ...
            $New_user = $this->input->post('user');
            $sub_user = substr($user, -4); //ตัดเอา4ตัวหลัง
            $amount_out = $this->input->post('amount'); //จำนวนที่ต้องการเพิ่้ม
            $query = $this->db->select('*')->where('user', $user)->get('tb_user')->result_array();
            ...
            foreach ($query as $result_user_point1) {
                ...
                // // เก็บ log ประวัติการเพิ่ม point ที่ตาราง  log_poin_and_spint
                //     $data = array(
                //         'user_id' => $sub_user,
                //         'admin_id' => $id_admin,
                ...
                //         'status' => '1',
                //     );
                //     $this->db->insert('log_poin_and_spin', $data);
                ...
                if ($result_point) {
                    ...
                    $time = time();
                    $sql = "INSERT INTO log_edit_user_point (admin, user_id,action,detail,datetime) 
                    VALUES ('" . $this->session->userdata['admin']['username'] . "', '" . $user . "', 'up','" . 'old : ' . $result_user_point1['point'] . ' new : ' .  $add_point . "','" . $time . "')";
                    if ($this->backend_library->query_sql($hostname, $dbname, $sql)) {
            ...
            foreach ($query as $result_user_spin1) {
                ...
                if ($amount_out <= $result_user_spin1['spin']) {
                    ...
                    if ($result_q) {
                        ...
                        $dbname = $dbname[0]['database_name'];
                        $time = time();
                        $sql = "INSERT INTO log_edit_user_spin (admin, user_id,action,detail,datetime) 
                            VALUES ('" . $this->session->userdata['admin']['username'] . "', '" . $user . "', 'down','" . 'old : ' . $result_user_spin1['spin'] . ' new : ' .  $add_result . "','" . $time . "')";
                        if ($this->backend_library->query_sql($hostname, $dbname, $sql)) {

                           
                        } else {

                            $sql = "CREATE TABLE log_edit_user_spin (
								id INT(10) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
								admin VARCHAR(20) NOT NULL,
								user_id VARCHAR(20) NOT NULL,
                                action VARCHAR(5) NOT NULL,
        ...
        if ($this->input->post('user') && $this->input->post('amount')) {
            ...
            foreach ($query as $result_user_spin1) {
                ...
                if ($result_point) {
                    ...
                    $time = time();
                    $sql = "INSERT INTO log_edit_user_spin (admin, user_id,action,detail,datetime) 
                    VALUES ('" . $this->session->userdata['admin']['username'] . "', '" . $user . "', 'up','" . 'old : ' . $result_user_spin1['spin'] . ' new : ' .  $add_result . "','" . $time . "')";
                    if ($this->backend_library->query_sql($hostname, $dbname, $sql)) {
    ...
    public function edit_salename()
    {
        ...
        $sale_name = $this->db->where('id', $sale_id)->where('status', 1)->get('tb_sale')->row()->name;
        $user_s = $this->db->where('user_id', $id)->get('tb_sale_user');
        $user = $this->db->where('id', $id)->where('status', 1)->get('tb_user')->row()->user;
        ...
        if ($user_s->num_rows() != 0) {
            ...
            if ($update_s = $this->db->set('sale_id', $sale_id)->where('user_id', $id)->update('tb_sale_user')) {
                ...
                $dbname = $dbname[0]['database_name'];
                $time = time();
                $sql = "INSERT INTO log_edit_user_sale (admin, user_id, sale,datetime) 
                VALUES ('" . $this->session->userdata['admin']['username'] . "', '" . $user . "', '" . $sale_name . "','" . $time . "')";
                if ($this->backend_library->query_sql($hostname, $dbname, $sql)) {

                  
                } else {

                    $sql = "CREATE TABLE log_edit_user_sale (
								id INT(10) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
								admin VARCHAR(20) NOT NULL,
								user_id VARCHAR(20) NOT NULL,
    ...
    public function reset_turnover()
    {
        ...
        $tb_user = $this->db->where('user', $user_id)->get('tb_user');
        $id = $tb_user->row()->id;
        $user = $tb_user->row()->user;
        $tb_turnover = $this->db->where('user_id', $id)->get('tb_turnover');
        $turn = $tb_turnover->row()->checkturn;
        ...
        if ($turn != '' & $user_id != '') {
            if ($turn > 0) {
                ...
                $this->db->where('user_id', $id)->update('tb_turnover', $data);
                /*--------------------------log_edit_user_turn----------------------------------- */
                $webname = $this->db->where('name', 'web')->get('setting')->row()->code;
                ...
                $dbname = $dbname[0]['database_name'];
                $time = time();
                $sql = "INSERT INTO log_edit_user_turn (admin, user_id,action, detail,datetime) 
                      VALUES ('" . $this->session->userdata['admin']['username'] . "', '" . $user . "','reset', '" . 'old : ' . $turn . ' new : 0' . '  ' . "','" . $time . "')";
                if ($this->backend_library->query_sql($hostname, $dbname, $sql)) {

                   
                } else {

                    $sql = "CREATE TABLE log_edit_user_turn (
                                  id INT(10) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
                                  admin VARCHAR(20) NOT NULL,
                                  user_id VARCHAR(20) NOT NULL,
                                  action VARCHAR(10) NOT NULL,
                ...
                /*--------------------------end log_edit_user_turn----------------------------------- */

                $re = array('code' => 1, 'msg' => 'ทำรายการสำเร็จ');
            } else {
                $re = array('code' => 0, 'msg' => 'ไม่มียอดเทิร์นค้าง');
            }
        } else {
    ...
    public function add_turn()
    {
        ...
        $id = $tb_user->row()->id;
        $user = $tb_user->row()->user; 
        if(!$this->db->where('user_id', $id)->get('tb_turnover')->row()){
            $this->db->insert('tb_turnover',array('user_id'=>$id,'promotion_id'=>0, 'code_id'=>0, 'sport'=>'0', 'casino'=>'0', 'game'=>'0', 'checkturn'=>'0', 'check_time'=>time(),'status'=>1));
        }
        $tb_turn = $this->db->where('user_id', $id)->where('status', 1)->get('tb_turnover');
       

        $before_turn = $tb_turn->row()->checkturn;

        if ($turn != '' & $user_id != '') {
            if ($turn > 0) {

                $after_turn =  $before_turn + $turn;
                $data = array(
                    'check_time' => time(),
                    'checkturn' =>  $after_turn,
                );
                ...
                $time = time();
                $sql = "INSERT INTO log_edit_user_turn (admin, user_id,action, detail,datetime) 
                VALUES ('" . $this->session->userdata['admin']['username'] . "', '" . $user . "','up', '" . 'old : ' . $before_turn . ' new : ' . $after_turn . '  ' . 'add_turn : ' . $turn . "','" . $time . "')";
                if ($this->backend_library->query_sql($hostname, $dbname, $sql)) {
                    ...
                    $this->backend_library->query_sql($hostname, $dbname, $sql);
                    $sql = "INSERT INTO log_edit_user_turn (admin, user_id,action, detail,datetime) 
                     VALUES ('" . $this->session->userdata['admin']['username'] . "', '" . $user . "','up', '" . 'old : ' . $before_turn . ' new : ' . $after_turn . '  ' . 'amount : ' . $turn . "','" . $time . "')";
                    $this->backend_library->query_sql($hostname, $dbname, $sql);
                }
                /*--------------------------end log_edit_user_turn----------------------------------- */
                ...
                $re = array('code' => 0, 'msg' => 'ใส่จำนวนเทิร์น');
            }
        } else {
            $re = array('code' => 0, 'msg' => 'ไม่สามารถทำรายการได้');
        }
