<?php
class WdController extends MY_Controller
{
			...
			if ($type == 'check') {
				if ($wd_r->status == 1 && $chk_bw_id_r->bank_id == 5) { //check Verify scb bank
					...
					if ($this->input->post('bw_id')) {
						...
						$login_scb = $this->Scb_wd_model->login();
						if ($login_scb) {
							$vertify = $this->Scb_wd_model->getVerify($wd_r->account, $us_r->api_id, $wd_r->amount);
...
//                            //เซฟว่าแอดมินคนไหนทำงาน
					...
					//print_r($vertify);
					//print_r($this->Bbl_wd_model->Check($wd_r->account,intval($us_r->api_id),1));
					//print_r($this->Bbl_wd_model->GetBalance());
					die();
				}
			}
		}
				...
				if ($wd_r->status == 1) {
					...
					if ($this->input->post('bw_id')) {
                        ...
                        { 
							...
							$login_scb = $this->Scb_wd_model->login();
							if ($login_scb) {
								$wd_scb_auto = $this->Scb_wd_model->Transfer($wd_r->account, $us_r->api_id, $wd_r->amount);
								if ($wd_scb_auto) {
		...
		if ($user_id) {
			...
			if ($user_q->num_rows() == 1) {
				...
				foreach($wd_history as $key=> $value){
					$wd_history[$i]->datetime = strtotime($value->CreationTime);
					$i++;
				}
