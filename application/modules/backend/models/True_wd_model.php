...
class True_wd_model extends CI_Model {
   ...
   public $config = array();
    ...
    public function __construct ($config = null) {
        ...
        //########## select database #########
        $query_true = $this->db->select('user,pass,pin_app')->from('tb_bank_web')->where('type',2)->where("bank_id",21)->where("status",3)->get()->result_array();// query user, password , pin app ของบัญชีทรูวอลเล็ท
        //echo "<pre>";
        if(!empty($query_true))//แก้ Error ของหน้า Withdraw
        {
            $config =  array(
    ...
    public function updateConfig ($name = null, $value = null) {
        ...
        if ((!isset($this->config["no_file"]) || !$this->config["no_file"]) && is_null($this->config_path) && isset($this->config["username"])) {
            ...
            //$this->setConfigPath('true_wd/'.$this->config["username"].".identity", true, false);// เซต path ที่อยู่ของไฟล์ token
            $this->setConfigPath('../true_wd/'.$this->config["username"].".identity", true, false);
            // die();
        }
        return $this->config;
    }
    ...
    public function GetTransaction ($limit = 50, $page = 1, $start_date = null, $end_date = null) {
        ...
        return $this->request("GET", "/history-composite/v1/users/transactions/history/?".$query, array(
            "Authorization" => strval($this->config["access_token"]),
            "Signature" => hash_hmac("sha256", rtrim($this->mobile_api_endpoint, "/")."/history-composite/v1/users/transactions/history/?".$query, $this->request_remote_key()),
            "X-Device" => $this->remote_key_id
        ));
    }
    ...
    public function GetTransactionReport ($report_id) {
        ...
        return $this->request("GET", "/history-composite/v1/users/transactions/history/detail/".$report_id, array(
            "Authorization" => strval($this->config["access_token"]),
            "Signature" => hash_hmac("sha256", rtrim($this->mobile_api_endpoint, "/")."/history-composite/v1/users/transactions/history/detail/".$report_id, $this->request_remote_key()),
            "X-Device" => $this->remote_key_id
        ));
    }
