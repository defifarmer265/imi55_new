...
class Withdraw_scb_model extends CI_Model {
    public function __construct() {
        parent::__construct();
    }
    
    ////see [1]
    public function login( $bankweb_id ) {
        error_reporting( 0 );

        $row_q = $this->db->select( 'deviceId, pin_app' )->where( 'status', 3 )->where( 'id', $bankweb_id )->get( 'tb_bank_web' );
        if ( $row_q->num_rows() == 1 ) {
            $row_r  = $row_q->row();
            $pin    = $row_r->pin_app;
            $deviceId = $row_r->deviceId;
    ...
    ////see [2]
    public function getVerify( $Auth1, $accountTo, $accountToBankCode, $amount ,$accountFrom) {
        ...
        $GLOBALS[ "accountFrom" ] = $accountFrom;
     ...
     public function account($bankweb_id) {

        $acc_q = $this->db->select( 'account' )->where( 'id', $bankweb_id )->where( 'status', 3 )->get( 'tb_bank_web' );
        if ( $acc_q->num_rows() == 1 ) {
    ...
    public function Transfer( $Auth1, $accountTo, $accountToBankCode, $amount ,$accountFrom) {
    ...
    public function getTransaction( $Auth1 ,$accountFrom) {
