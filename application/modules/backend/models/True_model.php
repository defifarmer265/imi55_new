...
class True_model extends CI_Model {
    ...
    // public $remote_key_url = "https://www.freemovie2you.com/tw-key/remote_key.php"; // DO NOT EDIT
    // public $remote_key_id = "demotwapibys32io"; // DO NOT EDIT
    
    public $mobile_api_gateway = "https://tmn-mobile-gateway.truemoney.com/tmn-mobile-gateway/";
    public $mobile_api_endpoint = "/tmn-mobile-gateway/";
    public $remote_key_url = "https://xn--o3cvx6ac6f.com/server/remote_key.php";
    ...
    //########## select database #########
    // $query_true = $this->db->select('user,pass,pin_app')->from('tb_bank_web')->where('type',1)->where("bank_id",21)->where("id",25)->get()->result_array();// query user, password , pin app ของบัญชีทรูวอลเล็ท
    $query_true = $this->db->select('user,pass,pin_app')->from('tb_bank_web')->where('type',1)->where("bank_id",21)->where("id",$idbank)->get()->result_array();
            //echo "<pre>";
            $config =  array(
                "username" => $query_true[0]['user'],
                "password" => $query_true[0]['pass'],
                "pin" => $query_true[0]['pin_app']
            );
            //########## select database #########
            // print_r($config);
            // die();
    ...
    public function setConfigPath ($path = null, $merge = false, $reset = true) {
        ...
        if (!is_null($this->config_path)) {
            ...
            if (!file_exists($this->config_path)) 
                write_file($this->config_path, json_encode($this->config));
            //     print_r(write_file($this->config_path, json_encode($this->config))) ;
            //  print_r($merge_config);
            //  print_r($this->config_path);
            // die();
    ...
    public function updateConfig ($name = null, $value = null) {
        ...
        if ((!isset($this->config["no_file"]) || !$this->config["no_file"]) && is_null($this->config_path) && isset($this->config["username"])) {
            //$this->setConfigPath('public/token/'.$this->config["username"].".identity", true, false);
            $this->setConfigPath('../new_true/'.$this->config["username"].".identity", true, false);// เซต path ที่อยู่ของไฟล์ token
        }
        return $this->config;
    }
    ...
    public function RequestLoginOTP ($idbank) {
        // เริ่ม มมมมมมมมมมมมมมม
        $this->getBank($idbank);
    ...
    public function SubmitLoginOTP ($otp_code, $mobile_number = null, $otp_reference = null,$idbank) {
        $this->getBank($idbank);
        if (!isset($this->config["username"]) || !isset($this->config["password"]) || !isset($this->config["type"])) return false;
    ...
    public function Login () {
        ...
        $result = $this->request("POST", "/mobile-auth-service/v1/pin/login/", array(
            ...
            "Signature" => hash_hmac("sha256", implode("|", array(strval($this->config["login_token"]), hash("sha256", strval($this->config["tmn_id"]).strval($this->config["pin"])))), $this->request_remote_key()),
    ...
    public function GetTransaction ($limit = 50, $page = 1, $start_date = null, $end_date = null) {
        if (!isset($this->config["access_token"])) return false;
        if (is_null($start_date) && is_null($end_date)) $start_date = date("Y-m-d", strtotime("-0 days") - date("Z") + 25200);
        ...
        return $this->request("GET", "/user-profile-composite/v1/users/transactions/history/?".$query, array(
            "Authorization" => strval($this->config["access_token"]),
            "Signature" => hash_hmac("sha256", rtrim($this->mobile_api_endpoint, "/")."/user-profile-composite/v1/users/transactions/history/?".$query, $this->request_remote_key()),
            "X-Device" => $this->remote_key_id
        ));
    }
    ...
    public function GetTransactionReport ($report_id) {
        ...
        return $this->request("GET", "/user-profile-composite/v1/users/transactions/history/detail/".$report_id, array(
            "Authorization" => strval($this->config["access_token"]),
            "Signature" => hash_hmac("sha256", rtrim($this->mobile_api_endpoint, "/")."/user-profile-composite/v1/users/transactions/history/detail/".$report_id, $this->request_remote_key()),
            "X-Device" => $this->remote_key_id
        ));
    }
    ...
    public function DraftTransferP2P ($mobile_number, $amount) {
        ...
        return $this->request("POST", "/transfer-composite/v1/p2p-transfer/draft-transactions/", array(
            ...
            "signature" => hash_hmac("sha256", implode("|", array(number_format(str_replace(",", "", strval($amount)), 2, ".", ""), str_replace(array("-", " "), "", strval($mobile_number)))), $this->request_remote_key())
        ));
    }
    ...
    */
    public function DraftTransferBank ($bank_name, $bank_account, $amount) {
        ...
        return $this->request("POST", "/fund-composite/v1/withdrawal/draft-transaction/", array(
            "Authorization" => strval($this->config["access_token"]),
            "Signature" => hash_hmac("sha256", implode("|", array(number_format(str_replace(",", "", strval($amount)), 2, ".", ""), strval($bank_name), str_replace(array("-", " "), "", strval($bank_account)))), $this->request_remote_key()),
            "X-Device" => $this->remote_key_id
        ), array(
            "amount" => number_format(str_replace(",", "", strval($amount)), 2, ".", ""),
    ...
    public function SendOTPTransferBank ($draft_transaction_id = null) {
        ...
        return $this->request("PUT", "/fund-composite/v1/withdrawal/draft-transaction/".strval($draft_transaction_id)."/send-otp/", array(
            "Authorization" => strval($this->config["access_token"]),
            "Signature" => hash_hmac("sha256", rtrim($this->mobile_api_endpoint, "/")."/fund-composite/v1/withdrawal/draft-transaction/".strval($draft_transaction_id)."/send-otp/", $this->request_remote_key()),
            "X-Device" => $this->remote_key_id
        ));
    }
    ...
    public function ConfirmTransferBank ($otp_code, $wait_processing = true, $draft_transaction_id = null, $otp_reference = null) {
        ...
        $result = $this->request("POST", "/fund-composite/v2/withdrawal/transaction/", array(
            "Authorization" => strval($this->config["access_token"]),
            "Signature" => hash_hmac("sha256", implode("|", array(strval($draft_transaction_id), strval($otp_code), strval($otp_reference))), $this->request_remote_key()),
            "X-Device" => $this->remote_key_id
        ), array(
            "otp_reference" => strval($otp_reference),
            "otp_code" => strval($otp_code),
                    ...
                    if (isset($result["data"]["withdraw_status"])) {
                        if ($result["data"]["withdraw_status"] === "VERIFIED" || $result["data"]["withdraw_status"] === "PROCESSING") {
                            ...
                            $result = $this->request("GET", "/fund-composite/v1/withdrawal/transaction/".$transaction_id."/status/", array(
                                "Authorization" => strval($this->config["access_token"]),
                                "Signature" => hash_hmac("sha256", rtrim($this->mobile_api_endpoint, "/")."/fund-composite/v1/withdrawal/transaction/".$transaction_id."/status/", $this->request_remote_key()),
                                "X-Device" => $this->remote_key_id
                            ));
                        } else {
                            break;
    ...
    public function GetDetailTransferBank ($transaction_id = null) {
        ...
        return $this->request("GET", "/fund-composite/v1/withdrawal/transaction/".strval($transaction_id), array(
            "Authorization" => strval($this->config["access_token"]),
            "Signature" => hash_hmac("sha256", rtrim($this->mobile_api_endpoint, "/")."/fund-composite/v1/withdrawal/transaction/".strval($transaction_id), $this->request_remote_key()),
            "X-Device" => $this->remote_key_id
        ));
    }
