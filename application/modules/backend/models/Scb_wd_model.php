...
class Scb_wd_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->ApiRefresh =$this->ApiRefresh();
	...
	{
		$deviceId_q = $this->db->select('deviceId')->where('id',21)->where('status',3)->get('tb_bank_web');
		if($deviceId_q->num_rows() == 1){
			$deviceId = $deviceId_q->row()->deviceId;
		}else{
			$deviceId = '';
		}
		return $deviceId;
	...
	{
		$ApiRefresh_q = $this->db->select('ApiRefresh')->where('id',21)->where('status',3)->get('tb_bank_web');
		if($ApiRefresh_q->num_rows() == 1){
			$ApiRefresh = $ApiRefresh_q->row()->ApiRefresh;
		}else{
			$ApiRefresh = '';
		}
		return $ApiRefresh;
	...
	public function account()
	{
		...
		$acc_q = $this->db->select('account')->where('id',21)->where('status',3)->get('tb_bank_web');
		if($acc_q->num_rows() == 1){
