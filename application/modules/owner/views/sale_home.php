                  ...
                  <table id="" class="table-bordered dt-responsive nowrap " cellspacing="0" width="100%">
                    <thead style="background-color:#12205F;color: #FFF">
					  <tr align="center">
                        ...
                        <th width="7%" rowspan="2"> รหัส</th>
                        <th rowspan="2"> Username  </th>
                        <th rowspan="2"> ชื่อเซลล์ </th>
						<th width="10%" rowspan="2"> ลูกค้าทั้งหมด</th>
                    ...
                    <tbody id="history">
                      ...
                      foreach ($sale as $_s => $ds) { ?>
                        <tr align="center">
                          ...
                          <td><?php echo $i; ?></td>
                          <td><?php echo 'S'.substr(("0000" . (intval($ds['id']))), -4); ?></td>
                          ...
                          <td> 
                              <button type="button" class="btn btn-secondary btn-sm" onClick="sel_desale('<?=$ds['id']?>')">
                              ข้อมูล
                            </button>
							...
							<button type="button" class="btn btn-warning btn-sm" onClick=" $('#m_edit_pass').modal(); $('#edp_sid').val('<?=$ds['id']?>');$('#edp_sname').val('<?=$ds['username']?>');">
                              <i class="fa fa-key"> </i>
                            </button>
                            <button type="button" class="btn btn-warning btn-sm" onClick=" $('#m_edit_sale').modal(); $('#eds_sid').val('<?=$ds['id']?>');$('#eds_username').val('<?=$ds['username']?>');$('#eds_username').val('<?=$ds['name']?>');">
                              แก้ไข
                            </button>
    
                               <button type="button" class="btn btn-danger btn-sm" onClick="edit_status(<?=$ds['id']?>,this)">
...
<div class="modal fade " tabindex="-1" role="dialog" id="m_edit_pass" aria-hidden="true">
  <div class="modal-dialog ">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">แก้ไขข้อมูลเซลล์</h4>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>
      <form class="form-label-left input_mask " >
        <div class="modal-body">
          <div class="x_content"> 
            <div class="col-md-12 col-sm-12  form-group has-feedback ">
              <input type="hidden" id="eds_sid">
              <input type="text" id="eds_username" class="form-control has-feedback-left"  >
              <span class="fa fa-user form-control-feedback left" aria-hidden="true"></span> </div>

            <div class="col-md-12 col-sm-12  form-group has-feedback">
				
              <input type="text" class="form-control has-feedback-left" id="eps_name" >
			<span class="fa fa-key form-control-feedback left" aria-hidden="true"></span> 
              <span class="fa fa-eye form-control-feedback right" aria-hidden="true" onClick="OC_edp_p()"></span> 
			  </div>
      ...
      <div class="modal-footer">
        <button onClick="edit_sale()" class="btn btn-primary">Save</button>
        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
      </div>
...
<script>
    ...
    function edit_pass() {
		$('#cover-spin').show();
		var id 	 = $('#eds_sid').val();
		var username = $('#eds_username').val();
		var name = $('#eds_name').val();
		swal({
				text: "คุณต้องการแก้ไขรายละเอียดพนักงานเซลล์",
				icon: "warning",
				buttons: true,
				dangerMode: true,
			})
...
///////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  function upDateLink(){
    ...
    swal({
            ...
            if (willDelete) {
    $.ajax({
        url: '<?=base_url()?>owner/sale/update_token',
        type: 'POST',
        dataType: 'json',
