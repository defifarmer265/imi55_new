...
<link href="<?php  echo $this->config->item('tem_backend_vendors'); ?>datatables.net-bs/css/dataTables.bootstrap.min.css" rel="stylesheet">
  ...
  <div class="row">
    <div class="col-md-12 col-sm-12 ">
      <div class="x_panel">
        <div class="x_title">
          <h2>พนักงานสำหรับเข้า Backend<small></small></h2>
                  ...
                  <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%" style="font-size: 14px;">
                    ...
                    <tbody>
                      ...
                      <tr style="text-align:center;">
                        ...
                        <td align="center"><?php if(!empty($license->systems) &&$license->systems ==1){echo '<i style="color:#3AED33;" class="fa fa-check"></i>';}else{ echo '<i style="color:#F51F23;" class="fa fa-close"></i>';}?></td>
                        <td>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <form class="form-label-left input_mask" id="form_creAdmin">
        <div class="modal-body">
          <div class="x_content">
       <div class="row"><h5>รายละเอียดพนักงาน</h5></div>
            <div class="row">
...
function edit_license(id)
{
  ...
  $.ajax({
    ...
    if(res.systems == 1){$('#lic_systems').prop('checked', true);}else{$('#lic_systems').prop('checked', false);}
...
function edit_typeclass(id)
{
  ...
  $.ajax({
    ...
    if(res.class == 0){ $('#class_admin0').parent().addClass( "checked" );} else{$('#class_admin0').parent().removeClass( "checked" );}
    if(res.class == 1){ $('#class_admin1').parent().addClass( "checked" );} else{$('#class_admin1').parent().removeClass( "checked" );}
    if(res.class == 2){ $('#class_admin2').parent().addClass( "checked" );} else{$('#class_admin2').parent().removeClass( "checked" );}
  })
