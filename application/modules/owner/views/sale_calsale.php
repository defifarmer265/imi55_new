...
<div class="row">
	<div class="col-md-11 col-sm-11 ">
		<div class="x_panel">
			<div class="x_title">
				<h2><a href="<?= base_url() ?>owner/sale/home"><i class="fa fa-chevron-left" aria-hidden="true"></i> Back</a> / <?= $sale->username ?> (<?= $sale->name ?>)</h2>
		...
		function report_sale() {
			...
			swal({
				...
				.then((willDelete) => {
					...
					if (willDelete) {
						$.ajax({
								url: '<?= base_url() ?>owner/sale/report_sale',
								type: 'POST',
								dataType: 'json',
								data: {
									d1: d1,
		...
		function sel_sale() {
			...
			swal({
				...
				.then((willDelete) => {
					...
					if (willDelete) {
						$.ajax({
								url: '<?= base_url() ?>owner/sale/calculate_',
								type: 'POST',
								dataType: 'json',
								data: {
									cost: cost,
							...
							.done(function(res) {
								...
								if (res.code == 1) {
									...
									for (var i = 0; i < conut; i++) {
										...
										content_fdps += '<td class="text-center" id="' + user[i]['user'] + '"><button class="" onclick="">TURNOVER</button></td>';
//                                        content_fdps += '<td>'+user[i]['total_turnover']+'</td>'
										content_fdps += '</tr>';
									}
									content_fdps += '</table>';
...
//                                    console.log(content_fdps);
		...
		function getturn(user) {
			...
			$.ajax({
					url: '<?= base_url('owner/sale/getturn') ?>',
					type: 'POST',
					dataType: 'json',
					data: {
						d1: $('#single_cal2').val(),
