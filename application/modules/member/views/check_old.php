...
.box-row{
   border: 1.180rem dotted #fffced !important;
    box-shadow: 0 0 5px 2px #402ce2;
    border-radius: 11px;
    background-color: #352c2cc7;
}
...
<header class="masthead bg-primary text-white text-center" style="padding-top: calc(6rem - 10px) !important; height: 90%;" id="tap_contact">
   ...
   <div class="container  mt-1 glow-on-hover box-row" style="margin-top: 0.05rem !important; ">
     <?php foreach($query as $data){ $status = $data['status'];?>
      ...
      <div class="row" style="margin-right: -.9rem; margin-left: -0.4rem;">
            ...
            <label  class="text-left" style="font-size:20px;color: #eaeaea;font-weight: lighter;">
               <div class="row">
                  <div class="col-12" style="margin-top:-30px;">
                     <img src="<?= $this->config->item('tem_frontend');?>img/checkin/head-check-in.png"style="max-width:100%;height:auto;">
                  </div>
         ...
         <div class="col-3  col-sm-2  shadow border" style="background-color: #eceded; font-size:14px; border: .125rem solid #f3d84f !important;padding:2px !important;box-shadow: 0 0 5px 2px #ffffff;">
            <?php   
               $date = strtotime(date('Y-m-').$i.' 00:00:00'); // วันที่ loop
               $datenow  = strtotime(date('Y-m-d 00:00:00'));//วันที่ปัจจุบัน
               // ถ่าเลยวันเช็คอินให้ขึ้นสีแดง
               $pointt = $this->db->where('user_id',$this->session->member->id)->where('create_time',$date)->where('type','checkin')->get('tb_point')->num_rows();
